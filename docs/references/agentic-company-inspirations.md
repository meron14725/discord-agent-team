...
### [ChatGPT共有会話: 技術記事探し](https://chatgpt.com/share/6aa6112f-8a00-83ee-b381-71a2d55ba60e)

各Botを語尾ではなく判断基準で区別し、役割ごとに `Identity`、`Character`、`Conversation`、`Voice`、`Examples` を持つ `PERSONA.md` を置く案を採用する。現在Discord接続済みの統括、上流、下流、情シスSREを初期対象とし、追加Botにも同じ構造と評価を必須にする。通常返信は短く、定型的な導入、過剰な同意、役割の毎回の名乗りを避け、質問は一度に1件までとする。

人格は作業判断そのものへ混ぜず、`作業結果 → 事実と状態を固定 → PERSONAで返信文を整形`という境界で適用する。良い例5件と悪い例5件を役割ごとに用意し、事実、実行状態、権限、承認、安全条件が整形前後で変わらないことを検査する。Humanizerは補助的な文章整形としてのみ扱い、人格や判断基準の正本にはしない。長期記憶、関係性の更新、人格の自己変更は初期実装へ含めず、保存範囲、忘却、訂正、承認、ロールバックを定義する後続案件に分ける。
...
3. 完了: 専門家から別担当への型付き引き継ぎを、統括の制御層で実行する。追加する専門役は1段に限定し、追加担当と元担当の確認は最大2往復まで許可する。自己委譲、既存担当への重複、第三の担当への横流し、3往復目を拒否し、同じ追加担当への複数依頼は1実行へ統合する。
